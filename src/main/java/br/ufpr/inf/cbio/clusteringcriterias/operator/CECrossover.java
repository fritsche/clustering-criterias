...
/**
 * This class implements a CE cluster ensemble for crossover
 *
 * @author Adriano F. Kultzak <adriano_fk@hotmail.com>
 */
public class CECrossover implements CrossoverOperator<PartitionSolution> {

    private final int numberOfRequiredParents;
    private final NDArray<double[]> featureArray;
//    private final Dataset dataset;

    public CECrossover() throws JepException {
...
//        this.dataset = dataset;
    ...
    public PartitionSolution cspa(PartitionSolution a, PartitionSolution b) {
        ...
        try (Interpreter interp = new SharedInterpreter()) {
            ...
            interp.exec("import Cluster_Ensembles as CE");
            interp.set("a_labels", a_labels);
            interp.set("b_labels", b_labels);
            interp.set("k",k);
            interp.exec("cluster_runs = np.vstack((a_labels,b_labels))");
            interp.exec("consensus_clustering_labels = CE.cluster_ensembles(cluster_runs,N_clusters_max = k)");
        ...
        for (int i = 0; i < a.getNumberOfVariables() - 1; i++) {
            child.setVariableValue(i, (Integer) al.get(i));
        }
